...
describe('TopicsSection', () => {
    ...
    it('renders correctly when expanded', () => {
        render(_jsx(TopicsSection, { ...defaultProps }));
    ...
    it('handles the transition between minimized and expanded states', () => {
        const { rerender } = render(_jsx(TopicsSection, { ...defaultProps, isMinimized: false }));
        let header = screen.getByRole('button', { name: /Selecione um tópico/i });
        expect(header).toHaveAttribute('aria-expanded', 'true');
        rerender(_jsx(TopicsSection, { ...defaultProps, isMinimized: true }));
        header = screen.getByRole('button', { name: /Selecione um tópico/i });
        expect(header).toHaveAttribute('aria-expanded', 'false');
    });
});
//# sourceMappingURL=TopicsSection.test.js.map