    ...
    switch (name) {
        case 'name':
            if (!value || (typeof value === 'string' && value.trim().length < 2)) {
                return t('form.validation.name.required', 'Nome é obrigatório (mín. 2 caracteres)');
            }
            break;
        case 'email':
        ...
        case 'subject':
            if (!value || (typeof value === 'string' && value.trim().length < 3)) {
                return t('form.validation.subject.required', 'Assunto é obrigatório (mín. 3 caracteres)');
            }
            break;
        case 'message':
            if (!value || (typeof value === 'string' && value.trim().length < 10)) {
                return t('form.validation.message.required', 'Mensagem é obrigatória (mín. 10 caracteres)');
            }
            break;
        case 'gdprConsent':
            if (!value) {
                return t('form.validation.gdprConsent.required', 'Deve aceitar a política de privacidade');
            }
            break;
    }
    return undefined;
 ...
 */
export const validateAllFields = (formData, t) => {
    const errors = {};
    ...
    if (gdprError)
        errors.gdprConsent = gdprError;
    return errors;
};
//# sourceMappingURL=contactFormValidation.js.map