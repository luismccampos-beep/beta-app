...
import type { SharedHeaderProps, SharedNavItem } from './navigation/types';
...
function filterItems(items: SharedNavItem[], userRole?: string, isAuthenticated?: boolean) {
...
export default function SharedHeader({
  ...
  const visibleItems = useMemo(
    () => filterItems(items, user?.role, Boolean(user)),
  ...
  return (
    ...
    >
      <div className="container flex h-16 items-center justify-between">
        <div className="flex items-center gap-3">
          <button
            ...
            className="flex items-center gap-2 border-0 bg-transparent cursor-pointer p-0 group"
          ...
          >
            <div className="h-8 w-8 rounded-lg bg-primary/10 flex items-center justify-center text-primary group-hover:bg-primary/20 transition-colors">
              {/* Placeholder logo icon if no logo component passed */}
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="hidden md:flex items-center gap-2 mr-2 border-l border-border/50 pl-4">
            <LanguageSwitcher
              showLabel={false}
      ...
      <SharedMobileMenu {...mobileMenuProps} />
    </header>
  );
}
