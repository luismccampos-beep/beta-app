...
interface DestinationFieldProps {
    ...
    error?: string | undefined;
    loading: boolean;
    suggestions: BookingDestination[];
    showSuggestions: boolean;
    onChange: (v: string) => void;
    onFocus: () => void;
    onBlur: () => void;
    onSelect: (dest: BookingDestination) => void;
}
...
interface DateRangeFieldsProps {
    ...
    loading: boolean;
    onChange: (range: BookingFormData['dateRange']) => void;
}

// FormErrors: every value is explicitly `string | undefined` for exactOptionalPropertyTypes
type FormErrors = {
...
const DEFAULT_DESTINATIONS: BookingDestination[] = [
    {
        id: 'lisboa',
        name: 'Lisboa',
        country: 'Portugal',
...
const FIELDS_TO_VALIDATE: (keyof BookingFormData)[] = [
...
function validateField(field: keyof BookingFormData, value: unknown): BookingValidationResult {
    switch (field) {
        case 'destination':
...
function validateAll(data: BookingFormData): FormErrors {
    ...
    for (const field of FIELDS_TO_VALIDATE) {
        ...
        if (!result.isValid && result.message) {
            ...
            switch (field) {
                case 'destination': errors.destination = result.message; break;
                case 'name': errors.name = result.message; break;
                case 'email': errors.email = result.message; break;
                case 'phone': errors.phone = result.message; break;
                case 'travelers': errors.travelers = result.message; break;
            }
        }
    }
...
function DestinationField({
    value, error, loading, suggestions, showSuggestions,
        ...
        <div className="space-y-1 relative">
            ...
            <div className="relative">
                ...
                {showSuggestions && suggestions.length > 0 && (
                    <ul className="absolute z-10 w-full bg-white border border-gray-300 rounded-lg mt-1 max-h-48 overflow-auto shadow-lg">
                        {suggestions.map((dest) => (
                            ...
                            >
                                ...
                                <div className="text-sm text-gray-500">{dest.country} — {dest.type}</div>
                            </li>
                        ))}
                    </ul>
                )}
...
function DateRangeFields({ dateRange, loading, onChange }: DateRangeFieldsProps) {
    ...
    const handleFromChange = (raw: string) =>
        onChange({ ...dateRange, from: raw ? new Date(raw) : undefined });

    const handleToChange = (raw: string) =>
        onChange({ ...dateRange, to: raw ? new Date(raw) : undefined });
...
function SelectField({ id, label, value, onChange, options, placeholder, loading, icon }: SelectFieldProps) {
    return (
        <div className="space-y-1">
            <label htmlFor={id} className="block text-sm font-medium text-gray-700">
                {label}
...
const BookingFormComponent = ({ onSubmit, loading = false }: BookingFormProps) => {
    ...
    const suggestions = useMemo(() => {
        if (formData.destination.length < 2) return [];
    ...
    const clearError = useCallback((field: keyof BookingFormData) => {
        setErrors((prev) => {
            switch (field) {
    ...
    const handleDestinationSelect = useCallback((dest: BookingDestination) => {
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-6 bg-white rounded-xl shadow-lg p-6">
            ...
            <FormField
                ...
                onChange={(v) => handleFieldChange('travelers', v)}
                placeholder="2"
                type="number"
            ...
            <SelectField
                ...
                onChange={(v) => handleFieldChange('travelType', v)}
            ...
            <div className="space-y-1">
                ...
                <textarea
                    ...
                    placeholder="Conte-nos mais sobre a sua viagem ideal..."
                    rows={3}
                    disabled={loading}
