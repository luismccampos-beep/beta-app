...
import { IndexedDBService, type ChatMessage } from '../../../../lib/db';

// Type for stored message data (may be in old format)
...
export class ChatStorageService {
  static async save(messages: Message[]): Promise<void> {
    ...
    try {
      ...
      await Promise.all(
        chatMessages.map(msg => IndexedDBService.put('chat', msg))
      );
    } catch (error) {
  ...
  static async load(): Promise<Message[]> {
    try {
      if (typeof window === 'undefined') return [];
  ...
  private static convertChatMessageToMessage(chatMsg: ChatMessage): Message | null {
    try {
      const metadata = chatMsg.metadata as Record<string, unknown> | undefined;
      ...
      if (metadata) {
        const messageMetadata: NonNullable<Message['metadata']> = {};
      ...
      if (Array.isArray(metadata?.attachments)) {
        message.attachments = metadata.attachments as Message['attachments'];
      }
  ...
  private static parseMessages(parsed: unknown): Message[] {
    if (!Array.isArray(parsed)) return [];

    const mapStoredMessage = (data: unknown): Message | null => {
      if (!data || typeof data !== 'object') return null;
      
      const msg = data as StoredMessageData;
      ...
      if (msg.metadata && typeof msg.metadata === 'object') {
        message.metadata = msg.metadata as Message['metadata'];
      }

      if (Array.isArray(msg.attachments)) {
        message.attachments = msg.attachments as Message['attachments'];
      }
