...
const BASE_CLASSNAMES: NonNullable<DayPickerProps['classNames']> = {
  ...
  head_row: 'flex',
  head_cell: cn(
    'w-10 rounded-md text-center text-[0.7rem] font-medium uppercase tracking-wider',
  ...
  day_today: cn(
    ...
    'font-semibold',
    'ring-1 ring-accent-foreground/20',
  ),
  day_outside: cn(
    'day-outside',
...
function IconLeft(props: React.SVGProps<SVGSVGElement>): React.ReactElement {
  return <ChevronLeft aria-hidden className={cn('size-4', props.className)} />;
}

function IconRight(props: React.SVGProps<SVGSVGElement>): React.ReactElement {
  return <ChevronRight aria-hidden className={cn('size-4', props.className)} />;
}
...
const Calendar = memo(function Calendar({
  ...
  const mergedClassNames: NonNullable<DayPickerProps['classNames']> = {
    ...BASE_CLASSNAMES,
    ...classNamesOverride,
  };
  ...
  const mergedComponents: DayPickerProps['components'] = {
