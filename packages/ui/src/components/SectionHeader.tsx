...
const getFlexAlignment = (align: string): string => {
  switch (align) {
...
const getSubtitleSizeClass = (size: string): string => {
  switch (size) {
...
const getAccentClass = (accentVariant: string): string => {
...
const SectionHeader = React.forwardRef<HTMLDivElement, SectionHeaderProps>(
  ({
    ...
    return (
      <div
        ref={ref}
        className={cn('relative flex flex-col gap-3 mb-10', alignClass, className)}
        {...props}
      >
        {/* Eyebrow decoration */}
        <div className={cn('flex items-center gap-2', flexAlignment)}>
          <span className="inline-block h-px w-8 bg-gradient-to-r from-blue-500 to-violet-500 rounded-full opacity-70" />
          <span className="inline-block h-1.5 w-1.5 rounded-full bg-blue-500 opacity-80" />
          <span className="inline-block h-px w-8 bg-gradient-to-l from-blue-500 to-violet-500 rounded-full opacity-70" />
        </div>

        {/* Heading */}
        <h2 className={cn('font-extrabold tracking-tight leading-[1.1] text-foreground', headingSizeClass)}>
          {title1 && <span>{title1} </span>}
          {title2 && <span className={accentClass}>{title2}</span>}
        </h2>

        {/* Decorative rule */}
        {showRule && (
          <div className={cn('flex', flexAlignment)}>
            <div className="h-1 w-16 rounded-full bg-gradient-to-r from-blue-500 to-violet-600" />
          </div>
        )}
        ...
        {subtitle && (
          <p className={cn(
            'text-muted-foreground leading-relaxed max-w-2xl font-normal',
            subtitleSizeClass,
