...
import {
  useAuthState as useAuthStateHook,
  useAuthOperations,
  useAuthSession,
  useAuthProfile,
  useAuthPassword,
...
const AuthContext = createContext<AuthState & AuthOperations | null>(null);
...
export function AuthProvider({
  ...
  const { refreshAccessToken, validateToken, checkSession, updateLastActivity } =
    useAuthSession(apiClient, authState, callbacks);

  // Profile operations hook
  const { updateProfile, getCurrentUser, deleteAccount } =
    useAuthProfile(apiClient, authState, callbacks);

  // Password operations hook
  const { changePassword, resetPassword, confirmResetPassword } =
    useAuthPassword(apiClient);
