// shared/src/themes/cssVars.ts
import type { Theme } from './types.js';

export interface CssVarOptions {
  prefix?: string; // e.g., 'akm'
  includeComments?: boolean; // Adiciona comentários nas CSS vars
}
 ...
 */
export function themeToCssVars(theme: Theme, options: CssVarOptions = {}): Record<string, string> {
  const prefix = options.prefix ? `${options.prefix}-` : '';
  const vars: Record<string, string> = {};

  // Colors/Palette (suporta objetos aninhados como primary.500)
  if (theme.palette) {
    Object.assign(vars, processNested(theme.palette as unknown as Record<string, unknown>, 'color', prefix));
  }
...
export function cssVarsString(
  ...
  options: { indent?: string | undefined; includeComments?: boolean | undefined } = {}
): string {
 ...
 * @param options - Opções de conversão
 */
export function generateThemeCss(theme: Theme, options: CssVarOptions = {}): string {
 ...
 */
export function cssVar(name: string, fallback?: string, prefix = 'akm'): string {
 ...
 * Útil para debugging
 */
export function extractCssVars(element: HTMLElement = document.documentElement): Record<string, string> {
