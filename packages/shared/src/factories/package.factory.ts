...
export const createManyReviews = (count: number): Review[] =>
  Array.from({ length: count }, () => createReview());

export const createFAQ = (overrides?: Partial<FAQ>): FAQ => ({
  id: faker.string.uuid(),
...
export const createManyFAQs = (count: number): FAQ[] =>
  Array.from({ length: count }, () => createFAQ());
...
export const createManyTestimonials = (count: number): Testimonial[] =>
  Array.from({ length: count }, (_, i) => createTestimonial({ index: i }));
...
export const createServiceAvailability = (overrides?: Partial<ServiceAvailability>): ServiceAvailability => ({
  date: faker.date.future().toISOString(),
  availableSlots: faker.number.int({ min: 0, max: 20 }),
  price: faker.number.int({ min: 50, max: 500 }),
  timeSlots: Array.from({ length: 3 }, () =>
    `${faker.number.int({ min: 8, max: 12 })}:00 - ${faker.number.int({ min: 13, max: 17 })}:00`
  ),
  ...overrides,
});

export const createAddOnService = (overrides?: Partial<AddOnService>): AddOnService => ({
  id: faker.string.uuid(),
...
export const createPackageService = (overrides?: Partial<PackageService>): PackageService => {
...
export const createManyPackageServices = (count: number): PackageService[] =>
  Array.from({ length: count }, () => createPackageService());
