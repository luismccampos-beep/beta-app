...
function AuthProvider({ children }) {
  ...
  const login = useCallback(
    async (emailOrCreds, password) => {
      updateState({ isLoading: true, error: null });
      try {
        const credentials = typeof emailOrCreds === "string" ? { email: emailOrCreds, password: password ?? "" } : emailOrCreds;
...
function useAuth() {
  ...
  if (!context) {
    throw new Error(
      "useAuth must be used within an AuthProvider. Make sure to wrap your app or component tree with <AuthProvider>."
    );
  }
  return context;
}
...
//# sourceMappingURL=chunk-34RS4VPE.js.map