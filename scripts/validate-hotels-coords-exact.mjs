 ...
 * 
 * Compares coordinates from "hoteis_coordenadas_exatas.csv" with wv_hotels in database
 * to detect discrepancies (hotels with same name but different coordinates).
 *
 * Usage:
 *   npm run travel:catalog:validate-hotels-coords-exact
 *   npm run travel:catalog:validate-hotels-coords-exact -- --max-distance-km 1
 *   npm run travel:catalog:validate-hotels-coords-exact -- --limit 1000
 */
...
async function loadExternalCsv() {
  const csvPath = join(process.cwd(), 'data', 'hotels', 'hoteis_coordenadas_exatas.csv');
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    // CSV format: "Nome do Hotel","Estrelas","Morada","Cidade","Pais","Latitude","Longitude","Notas"
...
async function main() {
  console.log('=== Hotel Coordinate Validation (Exact Coordinates) ===\n');
  ...
  const csvPath = join(outputDir, `hotel-coords-validation-exact-${timestamp}.csv`);
