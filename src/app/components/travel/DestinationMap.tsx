...
const DestinationMapLeaflet = dynamic(
  () => import('./DestinationMapLeaflet').then((m) => m.DestinationMapLeaflet),
  {
    ...
    loading: () => (
      <div
        className="h-[min(360px,50vh)] w-full animate-pulse rounded-xl bg-teal-100/50 dark:bg-teal-950/30"
        aria-hidden
      />
    ),
  },
...
export function DestinationMap({ data, markers: markersFromApi, className }: DestinationMapProps) {
  ...
  return (
    <Card
      className={
        className ??
        'overflow-hidden dark:bg-gray-800/80 border-teal-200/50 dark:border-teal-900/40'
      }
    >
      <CardHeader className="flex flex-row items-center justify-between gap-2 space-y-0 pb-2">
        ...
        <a
          ...
          className="text-xs text-teal-600 dark:text-teal-400 hover:underline inline-flex items-center gap-1 shrink-0"
        >
          {t('mapOpenOsm')}
      ...
      <CardContent className="space-y-2 p-3 pt-0">
        <div className="h-[min(360px,50vh)] w-full overflow-hidden rounded-xl border border-teal-100 dark:border-teal-900/50">
