...
function modeIcon(mode: string, provider: RoutingProvider) {
  const isTransit = mode.includes('bus') || mode.includes('rail') || mode.includes('subway') ||
  ...
  return (
    <div className="space-y-4">
      ...
      {plans[0] && !loading && (
        <div className="rounded-xl border border-violet-200 dark:border-violet-900/50 bg-violet-50/50 dark:bg-violet-950/20 p-4 space-y-3">
          <div className="flex flex-wrap items-center gap-3">
            ...
            {plans[0].carbon != null && (
              <span className="text-xs text-gray-500">{t('carbon')}</span>
            )}
          </div>
          {showDeparture && (
      ...
      {plans.length > 1 && !loading && (
        <p className="text-xs text-gray-400">{t('alternatives')}</p>
      )}
