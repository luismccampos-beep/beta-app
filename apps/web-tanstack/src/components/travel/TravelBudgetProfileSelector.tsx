...
import { useEnhancedTravelPreferencesFormTranslations as useTranslations } from '@/lib/i18n-namespaces';
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        {TRAVEL_BUDGET_PROFILE_IDS.map((id: TravelBudgetProfileId) => {
        ...
        <CardContent className="pt-5 space-y-4">
          <div className="flex flex-wrap items-center justify-between gap-2">
            ...
            <Badge variant="secondary" className="text-sm">
              {t('exampleCity').replace('{city}', estimate.referenceCity)}
            </Badge>
          </div>

          {/* Mobile: stacked card rows */}
          <div className="sm:hidden space-y-1">
            {estimate.lines.map((line: { id: string; quantity: number; subtotal: number }) => (
              <div
                key={line.id}
              ...
              >
                <span className="text-sm text-gray-800 dark:text-gray-200 flex items-center gap-1.5">
                  ...
                  {t(`lines.${line.id}`)}
                  <span className="text-xs text-gray-400 ml-1">x{line.quantity}</span>
                </span>
          ...
          <div className="hidden sm:block overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {estimate.lines.map((line: { id: string; quantity: number; unitPrice: number; subtotal: number }) => (
                  <tr
                    key={line.id}
