...
export function SandboxPreview({ onRegister, locale = 'pt', texts }: SandboxPreviewProps) {
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      const data = (await res.json().catch(() => ({}))) as { ok?: boolean; error?: string; destination?: unknown; day1?: unknown; stats?: unknown; totalDays?: unknown };
      ...
      setResult(data as unknown as SandboxResult);
      setState('result');
    } catch (err: unknown) {
      clearInterval(msgInterval);
