...
import { DestinationResultCard } from '@/components/travel/DestinationResultCard';
import { DestinationAirportBadge } from '@/components/travel/DestinationAirportBadge';
import { RecommendationsSection } from '@/components/travel/RecommendationsSection';
import { destinationDetailPath } from '@/lib/travel/destination-path';
import { orderTipSectionsForProfile, type TipSectionKey } from '@/lib/travel/destination-tips';
import {
...
export function ResultsPage({ onLogout, onNavigateToDashboard }: ResultsPageProps) {
  ...
  const t = useResultsTranslations();
  const tDest = useDestinationTranslations();
  const navigate = useNavigate();
  const search = useSearch({ strict: false });
  const searchParams = useMemo(() => new URLSearchParams(search as Record<string, string>), [search]);
  ...
  const patchSearchParams = useCallback(
    (updates: Record<string, string | null | undefined>) => {
      ...
      navigate({ search: Object.fromEntries(p) as never, replace: true });
    },
            ...
            {!resultsLoading && (
              <motion.div variants={staggerContainer} initial="hidden" whileInView="visible" viewport={{ once: true, margin: '-40px' }} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 md:gap-8">
                {filteredResults.map((result, idx) => {
                  ...
                  const tipPreviews = tipKeys.map((key: TipSectionKey) => {
