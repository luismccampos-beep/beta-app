...
import { useLocale as useI18nLocale, useTranslations as useAllTranslations } from '@/lib/i18n-provider';
import { useDestinationsBrowseTranslations } from '@/lib/i18n-namespaces';
import {
  Check,
  ChevronLeft,
  ChevronRight,
...
} from '@/components/travel/DestinationBrowseCard';
...
import { ACCOMMODATION_ICONS, ACCOMMODATION_COLORS } from '@/components/travel/accommodation-types';
...
export function DestinationsBrowsePage({ onBack }: DestinationsBrowsePageProps) {
  ...
  const t = useDestinationsBrowseTranslations();
  const allMessages = useAllTranslations();

  // ---- state ---------------------------------------------------------------

  // Filter & pagination state (synced to URL search params)
  const [query, setQuery] = useState((search as Record<string, string>)?.q ?? '');
  const [inputValue, setInputValue] = useState(query);
  const [continentFilter, setContinentFilter] = useState((search as Record<string, string>)?.continent ?? '');
  const [countryFilter, setCountryFilter] = useState((search as Record<string, string>)?.country ?? '');
  const [hotelTypeFilter, setHotelTypeFilter] = useState<string[]>(
    () => {
      const v = (search as Record<string, string>)?.hotelType;
      return v ? v.split(',').filter(Boolean) : [];
    },
  );
  const [page, setPage] = useState(Number((search as Record<string, string>)?.page ?? '1'));
  ...
  const { data: famousRaw, isLoading: famousLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const res = await fetch(`/api/travel/v1/destinations?${qp}`);        const data = (await res.json().catch(() => ({}))) as { ok?: boolean; items?: DestinationBrowseItem[]; total?: number };
  ...
  const continentList: { label: string; value: string }[] = useMemo(() => {
    try {
      const browseNs = allMessages.destinationsBrowse as Record<string, unknown> | undefined;
      const raw = browseNs?.continentOptions;
  ...
  const updateUrl = useCallback(
    (p: Record<string, string | undefined>) => {
      ...
      navigate({ search: params.toString() ? (Object.fromEntries(params) as never) : undefined, replace: true });
    },
    [navigate],
  );
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-primary-50 to-accent-50 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900 transition-colors">
      {/* AppHeader is rendered globally by __root.tsx — this page doesn't need its own */}
          ...
          <div className="flex items-center gap-2 mb-6 flex-wrap">
            ...
            {query && (
              <Badge variant="secondary" className="flex items-center gap-1.5 pl-2 pr-1 py-1 text-xs">
                ...
                <button type="button" onClick={() => { setInputValue(''); setQuery(''); }} aria-label={t('removeFilter').replace('{value}', query)} className="p-1.5 sm:p-0.5 rounded focus-visible:ring-2 focus-visible:ring-primary focus-visible:outline-none hover:bg-gray-200 dark:hover:bg-gray-600">
                  <X className="w-4 h-4" />
                </button>
              </Badge>
            )}
            {continentFilter && (
              <Badge variant="secondary" className="flex items-center gap-1.5 pl-2 pr-1 py-1 text-xs">
                ...
                <button type="button" onClick={() => { setContinentFilter(''); setPage(1); }} aria-label={t('removeFilter').replace('{value}', getContinentLabel(continentFilter))} className="p-1.5 sm:p-0.5 rounded focus-visible:ring-2 focus-visible:ring-primary focus-visible:outline-none hover:bg-gray-200 dark:hover:bg-gray-600">
                  <X className="w-4 h-4" />
                </button>
              </Badge>
            )}
            {countryFilter && (
              <Badge variant="secondary" className="flex items-center gap-1.5 pl-2 pr-1 py-1 text-xs bg-primary-100 dark:bg-primary-900/30 text-primary dark:text-primary-200">
                ...
                <button type="button" onClick={() => { setCountryFilter(''); setPage(1); }} aria-label={t('removeFilter').replace('{value}', countryFilter)} className="p-0.5 rounded focus-visible:ring-2 focus-visible:ring-primary focus-visible:outline-none hover:bg-primary-200 dark:hover:bg-primary-700">
                  <X className="w-4 h-4" />
                </button>
              </Badge>
            )}
            {hotelTypeFilter.map((tipo) => (
              <Badge key={tipo} variant="secondary" className="flex items-center gap-1.5 pl-2 pr-1 py-1 text-xs">
                ...
                <button type="button" onClick={() => { setHotelTypeFilter((prev) => prev.filter((t) => t !== tipo)); setPage(1); }} aria-label={t('removeFilter').replace('{value}', t(`accommodationTypes.${tipo}`) ?? tipo)} className="p-1.5 sm:p-0.5 rounded focus-visible:ring-2 focus-visible:ring-primary focus-visible:outline-none hover:bg-gray-200 dark:hover:bg-gray-600">
                  <X className="w-4 h-4" />
                </button>
              </Badge>
            ))}
                ...
                {Array.from({ length: Math.min(5, totalPages) }, (_, i) => {
                  ...
                  return (
                    <button type="button"
                      ...
                      aria-label={t('pageOf').replace('{page}', String(p)).replace('{total}', String(totalPages))}
                    >
                      {p}
                    </button>
                  );
