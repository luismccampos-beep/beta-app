...
import { useItineraryTranslations } from '@/lib/i18n-namespaces';
...
export function ItineraryPage({ slug }: ItineraryPageProps) {
  const t = useItineraryTranslations();
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        >
          <h1 className="text-3xl sm:text-4xl md:text-5xl font-black text-gray-900 dark:text-white mb-4 tracking-tighter break-words">
            {t('title').replace('{destination}', itinerary.destination)}
          </h1>
        ...
        >
          <QuickInfoCard
            icon={Calendar}
            label={t('duration')}
            value={t('duration_days').replace('{count}', String(itinerary.durationDays))}
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div className="min-w-0">
                      <h3 className="text-lg font-bold text-gray-900 dark:text-white truncate">
                        {t('day').replace('{day}', String(day.day)).replace('{title}', day.title)}
                      </h3>
                      <p className="text-sm text-gray-600 dark:text-gray-400">
                        {t('activities').replace('{count}', String(day.activities.length))}
                      </p>
                    </div>
                  </div>
                </div>
          ...
          <Card className="border-2 border-accent-200 dark:border-accent-700 shadow-xl overflow-hidden">
            ...
            <CardContent className="p-8 space-y-4">
              ...
              <Button
                ...
                onClick={() => navigate({ to: '/preferences/edit' })}
              >
                {t('ctaButton')}
              </Button>
            </CardContent>
