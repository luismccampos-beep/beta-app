...
import { useEnhancedTravelPreferencesFormTranslations } from '@/lib/i18n-namespaces';
...
export function QuickPreferencesForm() {
  ...
  const t = useEnhancedTravelPreferencesFormTranslations();
  ...
  useEffect(() => {
    ...
    fetch(`/api/travel/catalog?locale=${encodeURIComponent(locale)}`)
      .then((r) => r.json() as Promise<TravelCatalogResponse | null>)
  ...
  useEffect(() => {
    fetch('/api/travel/v1/destinations/countries')
      .then((r) => r.json())
      .then((data: unknown) => {
        const d = data as { countries?: FilterOption[]; continents?: FilterOption[] }
        if (d.countries?.length) setFilterCountries(d.countries);
        if (d.continents?.length) setFilterContinents(d.continents);
      })
      .catch(() => {});
  }, []);
  ...
  const onSubmit = async (data: QuickStartPreferences) => {
    ...
    try {
      ...
      const result2 = (await res.json().catch(() => ({}))) as { success?: boolean; message?: string };
      ...
      navigate({ to: '/results', search: { from: 'quickstart' } });
    } catch (err: unknown) {
